...
int GetBudgetPaymentCycleBlocks()
{
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        // TODO: remove the control flow for the creation of periodic
        // superblocks. Setting to integer max. for now since the max. MINTD
        // supply of 25 million would be generated in 4797556 blocks
        int intMax = std::numeric_limits<int>::max();
        return intMax;
    }

    // For testing purposes (one block per minute: create every hour)
    return 60;
}
...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf, bool fBudgetFinalization)
{
    ...
    BOOST_FOREACH (const CTxOut o, txCollateral.vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 25 MINTD finalization collateral
            //       will also cover the old 50 MINTD finalization collateral.
