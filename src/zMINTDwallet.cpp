...
CzMINTDWallet::CzMINTDWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zMINTD seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMINTDSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMINTDSeed_deprecated()) {
                    LogPrintf("%s: Updated zMINTD seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zMINTD seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zMINTD wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMINTDWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMINTDCount(nCountLastUsed);
    else if (!walletdb.ReadZMINTDCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMINTDWallet::Lock()
{
    seedMaster = 0;
}

void CzMINTDWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMINTDWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMINTD(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMINTDWallet::LoadMintPoolFromDB()
{
...
void CzMINTDWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMINTDWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMINTDWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zMINTDTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMINTDWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMINTD(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zMINTDTracker which also adds to database
    pwalletMain->zMINTDTracker->Add(dMint, true);
...
void CzMINTDWallet::SeedToZMINTD(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMINTDWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMINTDWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZMINTDCount(nCountLastUsed);
...
void CzMINTDWallet::GenerateDeterministicZMINTD(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMINTDWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMINTDWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
