...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zMINTDTracker = unique_ptr<CzMINTDTracker>(new CzMINTDTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZMINTD_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZMINTD_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
