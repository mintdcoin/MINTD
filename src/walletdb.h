...
public:
    ...
    bool WriteZMINTDSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMINTDSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMINTDSeed_deprecated(uint256& seed);
    bool EraseZMINTDSeed();
    bool EraseZMINTDSeed_deprecated();

    bool WriteZMINTDCount(const uint32_t& nCount);
    bool ReadZMINTDCount(uint32_t& nCount);
