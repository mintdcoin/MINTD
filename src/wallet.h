...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 MINTD at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zMINTD send
enum ZerocoinSpendStatus {
    ZMINTD_SPEND_OKAY = 0,                            // No error
    ZMINTD_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMINTD_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMINTD_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMINTD_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMINTD_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMINTD_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMINTD_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMINTD_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMINTD_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMINTD_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMINTD_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMINTD_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMINTD_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMINTD_SPENT_USED_ZMINTD = 14,                      // Coin has already been spend
    ZMINTD_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZMINTD_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZMINTDBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZMINTDOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzMINTDAuto) const;
    ...
    */
    ...
    CzMINTDWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzMINTDTracker> zMINTDTracker;
    ...
    void setZWallet(CzMINTDWallet* zwallet)
    {
        zwalletMain = zwallet;
        zMINTDTracker = std::unique_ptr<CzMINTDTracker>(new CzMINTDTracker(strWalletFile));
    }

    CzMINTDWallet* getZWallet() { return zwalletMain; }
    ...
    void setZMINTDAutoBackups(bool fEnabled)
    {
