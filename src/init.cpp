...
CzMINTDWallet* zwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "mintd.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "mintdd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the MINTD and zMINTD money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 19991, 19993));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in MINTD/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in MINTD/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, mintd, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in MINTD/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all MINTD specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:19991"));
    ...
    strUsage += HelpMessageOpt("-backupzMINTD=<n>", strprintf(_("Enable automatic wallet backups triggered after each zMINTD minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zMINTDbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zMINTD backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif
//    strUsage += "  -anonymizemintdamount=<n>     " + strprintf(_("Keep N MINTD anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 19992, 19994));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR))
      ...
      + "\n\n"
      + FormatParagraph(_("Copyright (C) 2019 The MINTD Developers"))
      + "\n\n"
...
/** Sanity checks
 *  Ensure that MINTD is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MINTD Core is shutting down."));
...
#endif
    // Make sure only a single MINTD process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MINTD Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //MINTD specific: zerocoin and spork DB's
                ...
                // MINTD: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZMINTDMinted();
                        RecalculateZMINTDSpent();
                    }
                    RecalculateMINTDSupply(1);
                }
                ...
                // MINTD: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MINTD Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart MINTD Core to complete") << "\n";
        ...
        zwalletMain = new CzMINTDWallet(pwalletMain->strWalletFile);
        ...
        uiInterface.InitMessage(_("Syncing zMINTD wallet..."));

        bool fEnableZMINTDBackups = GetBoolArg("-backupzMINTD", true);
        pwalletMain->setZMINTDAutoBackups(fEnableZMINTDBackups);
        ...
        pwalletMain->zMINTDTracker->Init();
...
//    }
//
//    nAnonymizeMINTDAmount = GetArg("-anonymizemintdamount", 0);
//    if (nAnonymizeMINTDAmount > 999999) nAnonymizeMINTDAmount = 999999;
//    if (nAnonymizeMINTDAmount < 2) nAnonymizeMINTDAmount = 2;
    ...
    LogPrintf("Anonymize MINTD Amount %d\n", nAnonymizeMINTDAmount);
    ...
    /* Denominations
       ...
       For example:
       1MINTD+1000 == (.1MINTD+100)*10
       10MINTD+10000 == (1MINTD+1000)*10
    */
