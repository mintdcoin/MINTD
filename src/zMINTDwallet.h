...
#ifndef MINTD_ZMINTDWALLET_H
#define MINTD_ZMINTDWALLET_H
...
public:
    CzMINTDWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMINTD(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMINTD(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //MINTD_ZMINTDWALLET_H
