...
#if defined(NDEBUG)
#error "MINTD cannot be compiled without assertions."
#endif
...
/**
 * Fees smaller than this (in uMINTD) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (pindex->nHeight > Params().Zerocoin_LastOldParams()) {
        ...
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zMINTD without the correct spend type. txid=%s\n", __func__,
                tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zMINTD spend with serial %s is already in block %d\n", __func__,
    ...
    if (!spend.HasValidSerial(paramsToUse))
        return error("%s : zMINTD spend with serial %s from tx %s is not in valid range\n", __func__,
...
bool CheckZerocoinSpend(const CTransaction tx, bool fVerifySignature, CValidationState& state, int nHeight)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (fVerifySignature) {
            ...
            if (!newSpend.Verify(accumulator, true)) // value of true is equivalent to !isBlockBetweenFakeSerialAttackRange(chainHeight)
...

    {
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zMINTD spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                             tx.GetHash().GetHex()),
                        REJECT_INVALID, "bad-txns-invalid-zMINTD");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zMINTD mints are not already known
            if (tx.IsZerocoinMint()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    if (nHeight < 200) {
        ...
        int64_t payment = blockValue * 0.8;
        return payment;
    }
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from MINTD
...
{
    RenameThread("mintd-scriptch");
    scriptcheckqueue.Thread();
}



void RecalculateZMINTDMinted()
{
...
void RecalculateZMINTDSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMINTD supply
...
bool RecalculateMINTDSupply(int nHeightStart)
{
...
bool UpdateZMINTDSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
    BlockToZerocoinMintList(block, listMints);
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    UpdateZMINTDSupply(block, pindex);
    ...
    //    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMINTDSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MINTD to zMINTD
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (nHeight > Params().Zerocoin_StartHeight()) {
        ...
        for (const CTransaction& tx : block.vtx) {
            ...
            // double check that there are no double spent zMINTD spends in this block
            if (tx.IsZerocoinSpend()) {
                for (const CTxIn txIn : tx.vin) {
                    if (txIn.scriptSig.IsZerocoinSpend()) {
                        libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                        if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                            return state.DoS(100, error("%s : Double spending of zMINTD serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zMINTD mints and %d zMINTD spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->cleanSubVer == "/MINTD Core:1.1.0/" ||
            pfrom->cleanSubVer == "/MINTD Core:1.3.0/" ||
            pfrom->cleanSubVer == "/MINTD Core:1.3.1/") {
            LOCK(cs_main);
        ...
        // MINTD: We use certain sporks during IBD, so check to see if they are
