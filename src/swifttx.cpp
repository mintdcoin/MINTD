...
bool ProcessConsensusVote(CNode* pnode, CConsensusVote& ctx)
{
    ...
    if (i != mapTxLocks.end()) {
        ...
        if ((*i).second.CountSignatures() >= SWIFTTX_SIGNATURES_REQUIRED) {
            ...
            if (!CheckForConflictingLocks(tx)) {
#ifdef ENABLE_WALLET
                if (pwalletMain && pwalletMain->UpdatedTransaction((*i).second.txHash)) {
                    nCompleteTXLocks++;
                }
#endif
    ...
    */
    BOOST_FOREACH (const CTxIn& in, tx.vin) {
        if (mapLockedInputs.count(in.prevout) && mapLockedInputs[in.prevout] != tx.GetHash()) {
            LogPrintf("SwiftX::CheckForConflictingLocks - found two complete conflicting locks - removing both. %s %s",
                      tx.GetHash().ToString().c_str(),
