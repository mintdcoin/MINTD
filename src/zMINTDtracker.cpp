...
CzMINTDTracker::CzMINTDTracker(std::string strWalletFile)
{
...
CzMINTDTracker::~CzMINTDTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMINTDTracker::Init()
{
...
bool CzMINTDTracker::Archive(CMintMeta& meta)
{
...
bool CzMINTDTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMINTDTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMINTDTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMINTDTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzMINTDTracker::GetSerialHashes()
{
...
CAmount CzMINTDTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMINTDTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMINTDTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMINTDTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMINTDTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMINTDTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMINTDTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMINTDTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMINTDTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMINTDTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMINTDTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMINTDTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMINTDTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMINTDTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMINTDTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMINTDTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMINTDTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzMINTD from DB\n", __func__, listDeterministicDB.size());
    }
