...
#include "ui_zMINTDcontroldialog.h"
...
std::set<std::string> ZMINTDControlDialog::setSelectedMints;
std::set<CMintMeta> ZMINTDControlDialog::setMints;

ZMINTDControlDialog::ZMINTDControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMINTDControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZMINTDControlDialog::~ZMINTDControlDialog()
{
    delete ui;
}

void ZMINTDControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMINTDControlDialog::updateList()
{
...
void ZMINTDControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMINTDControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZMINTD_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMINTDControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZMINTDControlDialog::GetSelectedMints()
{
...
void ZMINTDControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
