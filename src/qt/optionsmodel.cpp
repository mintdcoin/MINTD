...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MINTD);
    ...
    if (!settings.contains("nAnonymizeMINTDAmount"))
        settings.setValue("nAnonymizeMINTDAmount", 1000);

    nAnonymizeMINTDAmount = settings.value("nAnonymizeMINTDAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMINTDAmount"))
        SoftSetArg("-anonymizemintdamount", settings.value("nAnonymizeMINTDAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mintd.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMINTDAmount:
            return QVariant(nAnonymizeMINTDAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMINTDAmount:
            nAnonymizeMINTDAmount = value.toInt();
            settings.setValue("nAnonymizeMINTDAmount", nAnonymizeMINTDAmount);
            emit anonymizeMINTDAmountChanged(nAnonymizeMINTDAmount);
            break;
        case CoinControlFeatures:
