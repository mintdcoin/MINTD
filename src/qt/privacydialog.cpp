                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zMINTDpayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);
    

    // "Spending 999999 zMINTD ought to be enough for anybody." - Bill Gates, 2017
    ui->zMINTDpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMINTDSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMINTDpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMINTD_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMINTD is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMINTD...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMINTD in ") +
...
void PrivacyDialog::on_pushButtonSpendzMINTD_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMINTD is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMINTD
        sendzMINTD();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMINTD
    sendzMINTD();
}

void PrivacyDialog::on_pushButtonZMINTDControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZMINTDControlDialog* zMINTDControl = new ZMINTDControlDialog(this);
    zMINTDControl->setModel(walletModel);
    zMINTDControl->exec();
}

void PrivacyDialog::setZMINTDControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMINTDSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMINTD()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid MINTD Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMINTDpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMINTD is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MINTD </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMINTD</b>";
    ...
    // use mints from zMINTD selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMINTDControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZMINTDControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zMINTD require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zMINTD"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZMINTD_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zMINTD require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zMINTD"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zMINTD was spent successfully update the addressbook with the label
    ...
    // Clear zMINTD selector in case it was used
    ZMINTDControlDialog::setSelectedMints.clear();
    ui->labelzMINTDSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMINTD Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " MINTD, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMINTD Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zMINTDTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMINTD </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMINTD "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMINTD "));
    ui->labelzMINTDAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMINTDSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzMINTD->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zMINTD
        ui->pushButtonMintzMINTD->setEnabled(false);
        ui->pushButtonMintzMINTD->setToolTip(tr("zMINTD is currently disabled due to maintenance."));

        // Spend zMINTD
        ui->pushButtonSpendzMINTD->setEnabled(false);
        ui->pushButtonSpendzMINTD->setToolTip(tr("zMINTD is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zMINTD
        ui->pushButtonMintzMINTD->setEnabled(true);
        ui->pushButtonMintzMINTD->setToolTip(tr("PrivacyDialog", "Enter an amount of MINTD to convert to zMINTD", 0));

        // Spend zMINTD
        ui->pushButtonSpendzMINTD->setEnabled(true);
        ui->pushButtonSpendzMINTD->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}