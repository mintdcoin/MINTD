...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::MINTD)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sMINTDPercentage, QString& szMINTDPercentage)
{
    int nPrecision = 2;
    ...
    szMINTDPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sMINTDPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount MINTDAvailableBalance = balance - immatureBalance - nLockedBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = MINTDAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // MINTD labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, MINTDAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelMINTDPercent->setText(sPercentage);
    ui->labelzMINTDPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zMINTD.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in mintd.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in mintd.conf");
    }
    ...
    bool showMINTDAvailable = settingShowAllBalances || MINTDAvailableBalance != nTotalBalance;
    bool showWatchOnlyMINTDAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showMINTDPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyMINTDPending = watchUnconfBalance != 0;
    bool showMINTDLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyMINTDLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showMINTDAvailable || showWatchOnlyMINTDAvailable);
    ui->labelBalanceText->setVisible(showMINTDAvailable || showWatchOnlyMINTDAvailable);
    ui->labelWatchAvailable->setVisible(showMINTDAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showMINTDPending || showWatchOnlyMINTDPending);
    ui->labelPendingText->setVisible(showMINTDPending || showWatchOnlyMINTDPending);
    ui->labelWatchPending->setVisible(showMINTDPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showMINTDLocked || showWatchOnlyMINTDLocked);
    ui->labelLockedBalanceText->setVisible(showMINTDLocked || showWatchOnlyMINTDLocked);
    ui->labelWatchLocked->setVisible(showMINTDLocked && showWatchOnly);
    ...
    bool showzMINTDAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzMINTDUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzMINTDImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzMINTDAvailable);
    ui->labelzBalanceMatureText->setVisible(showzMINTDAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzMINTDUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzMINTDUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzMINTDImmature);
    ui->labelzBalanceImmatureText->setVisible(showzMINTDImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelMINTDPercent->setVisible(showPercentages);
    ui->labelzMINTDPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("MINTD")
    updateDisplayUnit();
}
